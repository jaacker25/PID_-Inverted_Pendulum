	...
	;This licence will expire on Sun, 01 Jul 2012
	...
	; Compiler options:
	;
	; -o16f684.cof -m16f684.map --summary=default --output=default c684.p1 \
...
;	15 bytes of RAM objects in bank 0 to zero

	psect	init
	global	__Lrbss_0
	movlw	low(__Lrbss_0)		;load the low address into FSR
	movwf	FSR
	movlw	low(__Lrbss_0+15)		;load the high address+1 into W
...
;	Sequence of clrf's more optimal than using clear routine
	clrf	(__Lcommon+0)		;clear byte at RAM address 0x77
	clrf	(__Lcommon+1)		;clear byte at RAM address 0x78
	clrf	(__Lcommon+2)		;clear byte at RAM address 0x79
	clrf	(__Lcommon+3)		;clear byte at RAM address 0x7A
	clrf	(__Lcommon+4)		;clear byte at RAM address 0x7B
	clrf	(__Lcommon+5)		;clear byte at RAM address 0x7C
